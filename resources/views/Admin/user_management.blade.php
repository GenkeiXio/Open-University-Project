    ...
    <div class="w-full">
        ...
        <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6 mt-4 mb-6">
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Manage all admins and faculty accounts</p>
            </div>
                    ...
                    <tbody class="text-gray-700 dark:text-gray-300">
                        @foreach($users as $user)
                            <tr class="border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                                <td class="px-6 py-4 font-medium">{{ $user->txt_fname }} {{ $user->txt_lname }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ $user->txt_email }}</td>
                                <td class="px-6 py-4">
                                    @if($user->txt_role == 'admin')
                                        <span class="px-3 py-1 text-xs rounded-full bg-purple-100 text-purple-600">Admin</span>
                                    @elseif($user->txt_role == 'faculty')
                                        <span class="px-3 py-1 text-xs rounded-full bg-blue-100 text-blue-600">Faculty</span>
                                    @else
                                        <span class="px-3 py-1 text-xs rounded-full bg-emerald-100 text-emerald-600">Staff</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-gray-700 dark:text-gray-300">{{ $user->txt_position ?? 'N/A' }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-gray-500">{{ $user->txt_lastlogin  ?? 'Never' }}</td>
                                <td class="px-6 py-4 text-gray-500">{{ $user->txt_lastlogout ?? 'Never' }}</td>
                ...
                <form method="POST" action="{{ route('admin.user.store') }}">
                    ...
                    <div class="flex justify-end gap-2 mt-5">
                        <button type="button" onclick="closeModal('createUserModal')"
                            class="px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-sm">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm">Create User</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div id="editUserModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div class="bg-white dark:bg-[#1e293b] w-full max-w-lg rounded-2xl shadow-xl p-6">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white flex items-center gap-2">
                        <i data-lucide="edit" class="w-5 h-5"></i> Edit User
                    </h3>
                ...
                <form method="POST" id="editUserForm">
                    ...
                    <div class="flex justify-end gap-2 mt-5">
                        <button type="button" onclick="closeModal('editUserModal')"
                            class="px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-600 text-sm">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm">Update User</button>
                    </div>
                </form>
            </div>
        </div>
